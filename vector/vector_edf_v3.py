"""Vectorized Holistic Local EDF analysis — v3 (per-scenario busy period).

Like v2 but computes busy periods L_i individually for each deadline
scenario, eliminating the over-estimation caused by using max-jitter
across scenarios.  Yields WCRTs and costs identical to the scalar
analysis for every perturbed scenario.
"""
...
def _busy_period_multi(C, T, J, same_proc, max_iters=200, tol=1e-6):
    """Compute busy periods for *s* scenarios simultaneously.

    J : (s, n) float32 — per-scenario jitter.
    Returns L : (s, n) float32.
    """
    s, n = J.shape
    L = np.tile(C.reshape(1, n), (s, 1)).astype(np.float32)
    j_mask = (same_proc & ~np.eye(n, dtype=bool)).astype(np.float32)
    T_brd = T.reshape(1, 1, n)
    C_brd = C.reshape(1, 1, n)
    j_mask_brd = j_mask.reshape(1, n, n)

    for _ in range(max_iters):
        own = np.ceil(L / T.reshape(1, n)) * C.reshape(1, n)              # (s, n)
        L_exp = L[:, :, np.newaxis]                                        # (s, n, 1)
        J_exp = J[:, np.newaxis, :]                                        # (s, 1, n)
        interference = np.sum(
            np.ceil((L_exp + J_exp) / T_brd) * C_brd * j_mask_brd, axis=2
        )                                                                  # (s, n)
        L_new = own + interference
        if np.allclose(L_new, L, atol=tol):
            return L_new.astype(np.float32)
        L = L_new
...
class VectorHolisticEDFAnalysisV3:
    ...
    def _analysis(self, D_scenarios, D_flow, C, T, J_init, pred_idx, same_proc):
        ...
        for outer in range(50):
            ...
            # Per-scenario busy periods
            L = _busy_period_multi(C, T, J, same_proc)                     # (s, n)
            P_vals = np.maximum(1, np.ceil(L / T.reshape(1, n)).astype(np.int32))  # (s, n)
            P_max = int(P_vals.max())

            # Per-scenario Q_max for interfering tasks
            Q_max_per_j = np.zeros((s, n), dtype=np.int32)
            for j in range(n):
                candidates = []
                for i in range(n):
                    if same_proc[i, j] and i != j:
                        candidates.append(
                            np.ceil((L[:, i] + J[:, j]) / T[j]).astype(np.int32)
                        )
                if candidates:
                    Q_max_per_j[:, j] = np.max(np.stack(candidates, axis=0), axis=0)
            Q_global = max(int(Q_max_per_j.max()), 1)

            for i in order:
                # Per-scenario max p for this task
                Pi_s = P_vals[:, i]                                          # (s,)
                Pi_max = int(Pi_s.max())
                if Pi_max == 0:
                    continue
                ...
                interfering = [j for j in range(n)
                               if same_proc[i, j] and j != i]

                wcrt_i = np.full(s, 0.0, dtype=np.float32)

                for p_idx in range(Pi_max):
                    p = p_idx + 1
                    # Only consider this p for scenarios where p <= P_vals[s,i]
                    p_ok = p <= Pi_s                                             # (s,)
                    ...
                    max_r = np.where(p_ok, r_own, -1e9)

                    # --- Interfering deadlines ---
                    for j in interfering:
                        Qj_s = Q_max_per_j[:, j]                                 # (s,)
                        Qj_max = int(Qj_s.max())
                        if Qj_max == 0:
                            continue

                        Tj = T[j]
                        q_vals = np.arange(Qj_max, dtype=np.float32)

                        psi_j = (q_vals * Tj)[None, :] - J[:, j:j+1] + D[:, j:j+1]  # (s, Qj_max)
                        in_int = (psi_j >= lower[:, None]) & (psi_j < upper[:, None])
                        valid_q = ((q_vals * Tj)[None, :] >= J[:, j:j+1] - 1e-4)
                        # Also mask: q must be < Q_max_per_j[s, j]
                        q_in_range = q_vals[None, :] < Qj_s[:, None]               # (s, Qj_max)
                        mask = in_int & valid_q & q_in_range

                        if not mask.any():
                            continue

                        w_j = _compute_w(
                            np.full((s, Qj_max), p * Ci, dtype=np.float32),
                            psi_j, p, Ci,
                            J, D, C, T, same_proc_i,
                        )
                    ...
                    for j in interfering:
                        ...
                        max_r = np.where(in_int_dl & p_ok, np.maximum(max_r, r_dl), max_r)
...
class VectorEDFGradientFunctionV3(GradientFunction):
    ...
    def _compute_costs(self, system, inputs):
        ...
        analysis = VectorHolisticEDFAnalysisV3(
